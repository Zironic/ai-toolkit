...
- [X] Fix: ensure `get_noise_prediction` also respects adapter device/dtype hints for Z-Image (VideoX) routing; latents, timesteps and control_context are now moved/cast to the adapter dtype/device before calling the controlnet to avoid Float/BFloat16 mismatches (test: `testing/test_zimage_control_dtype.py`).
...
- [ ] Training forward pass:
  ...
  - Note: Implemented model-side Z-Image routing via `StableDiffusion.get_noise_prediction(...)` and wired it into `predict_noise`; added unit and integration tests to assert control routing and tiling behavior.
