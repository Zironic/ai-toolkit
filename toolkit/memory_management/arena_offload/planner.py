...
def training_pinned_keys_for_keep_last(model, keep_last) -> set[str]:
...
def build_training_plan(model, arena, canonical_modules, device, config) -> dict:
    ...
    floor = min(sum(record.committed_bytes for record in records), int(DEFAULT_RESIDENT_FLOOR_GIB * GIB))
    ...
    target = resident_budget if not automatic else min(resident_budget, floor)
    ...
    return {
        ...
        "model_bytes": singleton_bytes + sum(record.committed_bytes for record in records),
