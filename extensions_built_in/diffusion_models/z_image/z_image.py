...
class ZImageModel(BaseModel):
    ...
    def load_model(self):
        ...
        transformer = None
        if self.te_only:
            # Resolve the extras path without materializing the transformer.
            if model_path.endswith(".safetensors") and base_model_path == model_path:
                base_model_path = self.single_file_extras_repo
            ...
            # Assistant weights must be merged before quantization.
            if self.model_config.assistant_lora_path is not None:
                self.load_training_adapter(transformer)
                if self.model_config.qtype == "qfloat8":
                    self.model_config.qtype = "float8"

            # The universal loader may have loaded or created a quantized model.
