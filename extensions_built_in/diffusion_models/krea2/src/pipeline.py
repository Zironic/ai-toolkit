...
class Krea2Pipeline:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Reference K/V is reusable only when isolated reference attention is
        # enabled. Reference batches stay on the sequential CFG path because
        # the cache batch shape must remain stable if batched CFG falls back.
        ...
        # do not contribute), but reads each weight once. Reference calls remain
        # sequential so the reference K/V cache keeps a stable batch shape.
        do_batch_cfg = do_cfg and bool(batch_cfg) and ref_latents is None
        print(
