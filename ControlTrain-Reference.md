...
- Training integration notes
  ...
  - Implemented helper: `StableDiffusion._predict_noise_zimage(...)` which tiles control images to match cap_feats, prepares `control_context`, calls the ControlNet and ensures cap_feats are passed only to the transformer. This helper is integrated into `predict_noise` and SDTrainer.
