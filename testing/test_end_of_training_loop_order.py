...
def test_end_of_training_loop_called_after_attach(monkeypatch):
    ...
    def fake_attach(self, loss_dict):
        events.append('attach')

    def fake_end(self):
        events.append('end')

    monkeypatch.setattr(SDTrainer, '_attach_noise_metrics_to_loss_dict', fake_attach)
    monkeypatch.setattr(SDTrainer, 'end_of_training_loop', fake_end)
    ...
    out = trainer.hook_train_loop(batch)

    assert out is not None
    assert events == ['attach', 'end']
