...
training correctness, or steady-state performance.

Use `--load-mode production-model-load` only when explicitly validating the
production generic model-load session, including checkpoint startup time and
host-RAM behavior. The intentionally verbose
`YesIWantToCauseTBOfPagingOnPurposeBecauseImExplicitlyBenchmarkingDiskLoad`
mode preserves legacy load-then-copy behavior for an explicit paging/load
regression test; never select it for routine validation.
