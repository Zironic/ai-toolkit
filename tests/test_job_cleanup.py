...
def test_base_job_cleanup_retains_processes_until_failure_is_retried():
    calls = []
    ...
    assert job.process == processes
    assert all(process.job is not None for process in processes)

    processes[1].error = None
    job.cleanup()

    assert calls == ["second", "first", "second", "first"]
    assert job.process == []
